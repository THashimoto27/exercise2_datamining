...
# Linear regression
model_lmcv = map(saratoga_folds$train, ~ lm(log(price)~ ., data=.))
rmse_lmcv = mean(map2_dbl(model_lmcv, saratoga_folds$test, modelr::rmse))

# Knn
model_knncv = foreach(k = 1:200, .combine='rbind') %do% {
  models1 = map(saratoga_folds$train, ~ knnreg(log(price)~ ., k=k, data = ., use.all=FALSE))
...
Also, We tried to do the same regression on the way of K-CV, and then we got the rmse of the linear regression is 0.292 and that of the KNN regression is 0.299. This results looks like the same as the way of hte ten-times average.
